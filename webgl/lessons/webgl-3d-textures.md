...
You might have noticed a call to `gl.generateMipmap`. When we loaded the texture. What is that for?
...
Now imagine we tried to draw that texture on a polygon 2x2 pixels big on the screen. What color should
...
move it has to pick pixels from the largest image.
...
needing to read 32 pixels for ever pixel drawn. That's going to be slow. Another reason is if you're trying
to achieve a certain effect. For example if you want something to have that pixelated *retro* look maybe you
want to use `NEAREST`.  Mips also take memory. If you are never going to draw something smaller than the
largest mip why waste memory for those mips. Instead just use `NEAREST` or `LINEAR` as they only ever
use the first mip.
...
(use one sample that can adjust based on URL?)
...
Why doesn't the keyboard texture show up? That's beacuse WebGL has severe restrictions on textures that are not
a power of 2 in both dimensions. Powers of 2 are 1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, etc.
...
To fix it we need to turn set the wrap mode to `CLAMP_TO_EDGE` and turn off mip mapping by setting filtering
